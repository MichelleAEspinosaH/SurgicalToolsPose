# ******************************************************************************
#  combined_viewer_v3.py
#
#  Extends combined_viewer_v2.py with a 3-D reference model built from a
...
#  Display windows
#  ---------------
#  RGB Camera        : live colour feed
#  Depth Viewer      : live rainbow depth map
...
def get_object_mask(color_image, yolo_model):
    ...
    Returns (None, None, None, None) if nothing valid is detected.
    """
...
def main():
    ...
    if yolo_model is None:
        ...
        try:
            ...
            print(f"Fine-tuned model not found. Using fallback: {FALLBACK}")
            print("Run train_yolo.py to enable the surgical-tools model.")
            print("Classes:", yolo_model.names)
        except Exception as e:
            print(f"Could not load fallback model ({FALLBACK}): {e}")
            yolo_model = None
    ...
    # --- Approximate camera matrix from first RGB frame ----------------------
